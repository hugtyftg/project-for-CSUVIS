import { scaleLinear } from './common';
...
function markovMobility(
    ...
    nodes.forEach((nodeI) => {
      nodes.forEach((nodeJ) => {
        if (adjacentMatrix[id(nodeI)][id(nodeJ)]) {
          let idealDistance =
            defaultDistance * shortestPathMatrix[id(nodeI)][id(nodeJ)];
  ...
  links.forEach((link) => {
    if (!oldLinksById[link.id]) {
      p0[idFromEdge(link.source, id)] +=
        1 / degree[idFromEdge(link.source, id)];
      p0[idFromEdge(link.target, id)] +=
        1 / degree[idFromEdge(link.target, id)];
    }
  });
  ...
  nodes.forEach((nodeI) => {
    ...
    nodes.forEach((nodeJ) => {
      ...
      if (
        shortestPathMatrix[id(nodeI)][id(nodeJ)] === 1 &&
        id(nodeI) !== id(nodeJ)
      ) {
