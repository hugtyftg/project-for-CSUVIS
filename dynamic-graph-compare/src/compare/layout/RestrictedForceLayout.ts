...
import { Edge, ForceLayoutOptions, Model, PointTuple } from '@antv/layout';
import { isArray, isFunction, isNumber, isObject } from '@antv/layout/lib/util';
import forceInABox from '@antv/layout/lib/layout/force/force-in-a-box';
import { Base } from '@antv/layout/lib/layout/base';
import { LAYOUT_MESSAGE } from '@antv/layout/lib/layout/constants';
import forceSimulation from '../simulation/DynamicForceSimulation';
import { forceDis, forceLinkDir, forceLinkLen, forcePos } from '../forces';

class RestrictedForceLayout extends Base {
  /** 向心力作用点 */
  ...
  /** 是否支持按类聚合 */
  public clustering: boolean | undefined;
  ...
  public init(data: Model) {
    ...
    self.edges = edges.map((edge) => {
      ...
      (Object.keys(edge) as (keyof Edge)[]).forEach((key: keyof Edge) => {
  ...
  public execute(reloadData?: boolean) {
    ...
    if (!simulation) {
      try {
        simulation = forceSimulation().nodes(nodes as any);
        ...
        simulation
          ...
          .force(
            'charge',
            d3Force.forceManyBody().strength(self.nodeStrength ?? 0.5)
          )
          .force('link', d3Force.forceLink().distance(0))
        ...
        // node restrictions
        if (this.restrictions.includes('node distance')) {
          simulation.force('dis', forceDis());
        } else if (this.restrictions.includes('node position')) {
          simulation.force('pos', forcePos());
        }
        ...
        if (edges) {
          ...
          // link restrictions
          if (this.restrictions.includes('link length')) {
            simulation.force('linkLen', forceLinkLen(edges as any));
          } else if (this.restrictions.includes('link direction')) {
            simulation.force('linkPos', forceLinkDir(edges as any));
          }
        }
  ...
  public overlapProcess(simulation: any) {
    ...
    // parameter of forceCollide is a radius
    simulation.force(
      'collisionForce',
